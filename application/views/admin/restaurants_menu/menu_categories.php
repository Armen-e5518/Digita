<div class="n_titleBl">
    <h1><?php echo $labels->content .' '. $labels->list;?></h1>
                ...
                <div class="WidthPercent20">
                    <div class="n_transparence">
                        <b class="options">
                            ...
                            <a title="<?php echo $labels->edit;?>" href="<?= site_url("admin/Restaurants_menu/get_category_group/{$item->id}/{$pageNum}"); ?>" class="edit_item">
                                <i class="fa fa-pencil"></i>
                            </a>
                            <?php
                            $notremovedids = $this->settings['ContentNotRemovedIds'];
                            if(!in_array($item->id,$notremovedids)) { ?>
                                <a title="<?php echo $labels->trash;?>" class="remove-btn delete" href="<?= site_url("admin/{$mod}/remove/{$item->id}/{$pageNum}"); ?>">
                                    <i class="fa fa-trash-o"></i>
                                </a>
                            <?php } else { ?><i class="fa"></i> <?php } ?>
                        </b>
                    </div>
                </div>
            </li>
